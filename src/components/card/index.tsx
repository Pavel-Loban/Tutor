...
import { setBookId} from '../../store/booking-slice';
// import { Book} from '../../store/book-slice';
import { Book,setLoading, setModalCalendar} from '../../store/books-slice';
...
export const Card: React.FC<Book> = ({ image, id, title, authors, issueYear, booking, delivery, rating, categories, histories }) => {
    ...
    const { bookId } = useAppSelector((state: RootState) => state.booking);
    ...
    const {booksCategories, books} = useAppSelector((state: RootState) => state.books);
            ...
            >
                ...
                <div className={styles.book_footer}>
                    ...
                    onClick={e => e.stopPropagation() } role='presentation'>
                        ...
                        <Button handleClick={getModalCalendar}  buttonText={delivery === null && booking === null ? 'ЗАБРОНИРОВАТЬ' : (booking !== null ? `ЗАНЯТА ДО ${dayOrder}.${monthOrder}` : 'ЗАБРОНИРОВАНО')} delivery={delivery} booking={booking} order={booking?.order} />
                    </div>
                </div>
            </section> :
